...
{
    device = SDCARD; //SD card(здесь SSP инит!!!!)
...
void _myi2s_clk(void)
{
 
  CGU_Stop_Clock(CLK_256FS);
  CGU_Run_Clock(CLK_256FS);

  CGU_Stop_Clock(I2S_CFG_PCLK);
  CGU_Run_Clock(I2S_CFG_PCLK);
  CGU_Stop_Clock(EDGE_DET_PCLK);
  CGU_Run_Clock(EDGE_DET_PCLK);
  CGU_Stop_Clock(I2STX_FIFO_1_PCLK);
  CGU_Run_Clock(I2STX_FIFO_1_PCLK);
  CGU_Stop_Clock(I2STX_IF_1_PCLK);
  CGU_Run_Clock(I2STX_IF_1_PCLK);

  CGU_Stop_Clock(I2S_EDGE_DETECT_CLK);
  CGU_Run_Clock(I2S_EDGE_DETECT_CLK);
  CGU_Stop_Clock(I2STX_BCK1_N);
  CGU_Run_Clock(I2STX_BCK1_N);
  CGU_Stop_Clock(I2STX_WS1);
  CGU_Run_Clock(I2STX_WS1);
}

void _set_master(void)
{
#if 0  
  CGU_Run_Clock(I2SRX_FIFO_1_PCLK);
  CGU_Run_Clock(I2SRX_IF_1_PCLK);
  CGU_Run_Clock(I2SRX_BCK1);
  CGU_Run_Clock(I2SRX_BCK1_N);
  CGU_Run_Clock(I2SRX_WS1);
#endif  
  
  I2S_FORMAT_SETTINGS_bit.dai1_format = 3;
  I2S_FORMAT_SETTINGS_bit.dao1_format = 3;

  AUDIOSS_MUX_SETTINGS_bit.dai1_oe_n = 1; //rx slave 
}
...
{
unsigned char *_p;
  _p = (unsigned char *)_STATUS_REG;
  
  
  if((*((unsigned char *)_p)) & (0x1<<4))
    return 0x01;
...
void main(void)
{
  ...
  CP15_ICache(TRUE);           // DEnable ICache,DCache
  CP15_DCache(TRUE);
  
  _int_rec = 0x00;
  
	unsigned char _res_char;

    if(_int_play())
    {
      _int_rec = 0x01;
      _init_param(); //init BEGIN_BLOCK and etc
      _res_char = _init_nand_w();
      if(_res_char)
        while(1) asm("  NOP  ");        
    }
  ...
  CP15_ICache(FALSE);           // DEnable ICache,DCache
  CP15_DCache(FALSE);  
 // Init MMU
 // Privileged permissions  User permissions AP
  ...
  CGU_Run_Clock(DMA_PCLK);
  CGU_Run_Clock(DMA_CLK_GATED);
  ...
  //_stretch_init(1.0);
  _init_boost();
  init_IO(); //init_timer() вызывается здесь
  keyb_Init();//инит дорожек
...
if(!_int_rec)
{
if(!((IOCONF_UART_PIN >> 1) & 0x00000001)) //_SD_EXIST
{
  device = SDCARD; //SD card(здесь SSP инит!!!!)
  if(efs_init(&efs,&device)!=0)
  {
...
else
{
     ...
     m_env21_ = 0x01; //статус нет карты
} 
}
else
{
  ...
  device = NAND; //откроем FS на flash 
...
unsigned char _prev_k1;  
  ...
  if(_get_key_block())
  {
    //клава блокирована
                  ...
                  if((((IOCONF_GPIO_PIN >> 14) << 2) & 0x0004) != 0x04)
                  {
  ...
  _en_irq();
              ...
              if(_int_play() && (_rec_list == 0)) //0->переход в radio
              {
                _play_rem("/SYS/_45.mp3\0",0x01);
                _iplay_id = _curr_param._id_radio;
              }
              if(_int_play() &&  (_rec_list == 1)) //2->переход в lin rec
              {
                _play_rem("/SYS/_44.mp3\0",0x01);
                _iplay_id = _curr_param._id_lin;
              }
              if(_int_play() &&  (_rec_list == 2)) //3->переход в mic rec
              {
                _play_rem("/SYS/_43.mp3\0",0x01);
                _iplay_id = _curr_param._id_mic;
              }
              
                if(_int_play() && (!_iplay_id))
                {
                  _play_rem("/SYS/_3.mp3\0",0x01); //стоп
                  _uninit_boost(); //выкл усилок
                    for(unsigned long _l=0;_l<0x0cfffff;_l++)
                      asm("NOP");       
                  _turnon(0);
...
//сервисный режим затирания внутренней памяти
//две правые кнопки должны быть нажаты
//установим pins as input   
              IOCONF_GPIO_M0_CLR = (0x0fff<<3);
              IOCONF_GPIO_M1_CLR = (0x0fff<<3);
      
              if((((IOCONF_GPIO_PIN >> 3)  << 9)  & 0x0200) /*&& (((IOCONF_GPIO_PIN >> 4)  << 10)  & 0x0400)*/)
              {
                  _init_param(); //init BEGIN_BLOCK and etc
...
//скажем фразу "Остановка на текущ. позиции" 
            ...
            {
              _reset_WD();
              if(((IOCONF_UART_PIN >> 1) & 0x00000001))
...
//поставлена задача перечислить режимы воспр
          if((m_env100 & 0x01) == 0x01)
          {
            ...
            while(1)
            {
              ...
              {
              case 0:
                _play_rem((unsigned char *)"/sys/_45.mp3",1);
                for(_l=0;_l<0x00ffffff;_l++)
                  asm(" NOP ");            
              break;
              case 1:
                _play_rem((unsigned char *)"/sys/_44.mp3",1);
                for(_l=0;_l<0x00ffffff;_l++)
                  asm(" NOP ");            
              break; 
              case 2:
                _play_rem((unsigned char *)"/sys/_43.mp3",1);
                for(_l=0;_l<0x00ffffff;_l++)
                  asm(" NOP ");            
              break;          
              };          
