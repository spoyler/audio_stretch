...
unsigned char *_p;
      ...
      memset(_p,0,4);
      *_p = GetValidBlockNumber(7); //7 модуль это keeper
      _p = (unsigned char *)_STATUS_REG;
      *_p=0;
      if(_p1==1)
      {
        *((unsigned char *)_p) |= 0x1<<5; //_boot2 должен загрузить 7 module keeper, а затем radio
#if 0
        _p = (unsigned char *)_LOAD_BLOCK_NUM; 
        memset(_p,0,4);
      ...
      if(_p1==2) //lin
      {
        _p = (unsigned char *)_LOAD_BLOCK_NUM;  
        memset(_p,0,4);
        *_p = GetValidBlockNumber(8); //_lin_mic
        _p = (unsigned char *)_STATUS_REG;        
        memset(_p,0,4);        
      }
      if(_p1==3) //mic
      ...
      if(_p1==4) //radio
      {
        _p = (unsigned char *)_LOAD_BLOCK_NUM;  
        memset(_p,0,4);
        *_p = GetValidBlockNumber(5); //radio
        _p = (unsigned char *)_STATUS_REG;        
        memset(_p,0,4);       
      }    
      if(_p1==5) //tts
      {
        _p = (unsigned char *)_LOAD_BLOCK_NUM;  
        memset(_p,0,4);
        *_p = GetValidBlockNumber(1); //radio
        _p = (unsigned char *)_STATUS_REG;        
        memset(_p,0,4);   
        *_p++ |= 0xaa;
        *_p++ |= 0x55;
      ...
      //memcpy(_src_buff,(unsigned char *)&m_place,sizeof(m_place));     
}
...
unsigned char _param_get()
{
    ...
    if((strstr(m_place._sample,(unsigned char *)"LKF\0") == NULL) &&
        (strstr(m_place._sample,(unsigned char *)"MP3\0") == NULL))
       return 0x01;
#endif      
