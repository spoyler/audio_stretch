...
//будем сбрасывать BCR дл€ Domain_id
void cgu_bcr(unsigned char Domain_id, unsigned char _en)
{
  UNS_32 bcr_id = cgu_DomainId2bcrid(Domain_id);
...
void cgu_reset_all_clks(void)
{
  ...
  for (i = 0; i < (CGU_SYSCLK_O_LAST + 1); i++)
  {
    ...
    /* set enable-outТs for only the following clocks */
