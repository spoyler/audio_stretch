...
#if 0    
    r0_val = start;
    __MCR(15,0,(UNS_32)r0_val,7,14,1);
#else
   _tci_loop();    
#endif    
#endif    
...
#if 0  
  asm (    "  MCR p15, 0, r0_val, c7, c10, 4   \n" ); /* Flush Write Buffer */
#else  
  __MCR(15,0,(UNS_32)r0_val,7,10,4);    
#endif  
#else
...
__asm__ volatile("    MOV r0, %0    " : : "r"(end): "r0");
    __asm("    MCR p15, 0, r0, c7, c10, 1    ");  /* Clean D-Entry */
...
#else
    r0_val = start;
    __MCR(15,0,(UNS_32)r0_val,7,10,1);    
#endif    
    memcpy(pstart, tbuf, CACHE_DLINESIZE - sextra);
  }
...
#else
    r0_val = start;
    __MCR(15,0,(UNS_32)r0_val,7,6,1);
#endif    
  }
#if 0  
  __asm (   " MCR p15, 0, r0_val, c7, c10, 4  \n"   ); /* Flush Write Buffer */
#else
#endif
  __MCR(15,0,(UNS_32)r0_val,7,10,4);
#else
