...
TDStretch   _TDStretch;

//#define _TEMPO  1.9//(max без искажений)
#define _TEMPO  1.0//(min без искажений)
...
static int _getClosest2Power(float value)
{
...
void TDStretch::calculateOverlapLength(uint overlapMs)
{
    ...
    newOvl = (uint)pow((double)2, (double)overlapDividerBits);
...
float TDStretch::calcCrossCorrMono(const float *mixingPos, const float *compare) const
{
    float corr;
    uint i;
...
float TDStretch::calcCrossCorrStereo(const float *mixingPos, const float *compare) const
{
    float corr;
    uint i;
