/***********************************************************************
* $Id: lpc313x_mci_driver.c 30870 2009-07-05 10:16:16Z anderslu $
*
...
static void prv_set_clock(UNS_32 speed)
{
  ...
  UNS_32 mmc_clk = 45000000;//cgu_get_clk_freq(CGU_SB_SD_MMC_CCLK_IN_ID);
  ...
  while (state < 100)
  {
    switch (state)
    {
      ...
      case 13:
        ...
        {
          Timer1_wait_ticks(MS_ACQUIRE_DELAY*10);
          status = mci_execute_command(pdev, command, 0, 0);
          ocr = pdev->response[0] | (ocr & OCR_HC_CCS);
        }
        if (ocr & OCR_ALL_READY)
...
int i;
  /* enable SD/MMC clock */
  CGU_Run_Clock(SD_MMC_HCLK);
  CGU_Run_Clock(SD_MMC_CCLK_IN);
  
  /* reset SD/MMC/MCI modules through CGU */
  SD_MMC_PNRES_SOFT = 0;
  SD_MMC_NRES_CCLK_IN_SOFT = 0;
  
  for (i = 0;i < 1000;i++);
  
  SD_MMC_PNRES_SOFT = 1;
  SD_MMC_NRES_CCLK_IN_SOFT = 1;

  /* Set IOCONF to MCI pins */
  SYS_REGS->mci_delaymodes = 0;
  SYS_REGS->mux_gpio_mci_sel = 1;
  /* set the pins as driven by IP in IOCONF */
  IOCONF_EBI_MCI_M1_CLR = 0xF0000003;
  IOCONF_EBI_MCI_M0_SET = 0xF0000003;
...
int i;
  ...
  CGU_Stop_Clock(SD_MMC_HCLK);
  CGU_Stop_Clock(SD_MMC_CCLK_IN);
  return _NO_ERROR;
}
...
//Checks whether the card can accept new data or whether the write process is still in progress
/*INT_32 mci_wait(INT_32 devid)
{
  int status;
  int timeout_cnt = US_TIMEOUT;
  MCI_CARD_INFO_T* pdev = (MCI_CARD_INFO_T*) devid;
   ...
   // wait until the card has processed all data 
    do {
      ...
      if (status & MCI_INT_RTO) //response timeout    
          return -1; // unable to get the card state. So return immediately.  
  
     if (pdev->response[0] & R1_READY_FOR_DATA) return 0; // 0x100 READY_FOR_DATA bit 
     Timer1_wait_ticks(1);
     timeout_cnt--;
  ...
  MCI_CARD_INFO_T* pdev = (MCI_CARD_INFO_T*) devid;
   ...
   // wait until the card has processed all data 
    do {
     ...
     if (status == SDMMC_STBY_ST || status == SDMMC_TRAN_ST) return 0;
     Timer1_wait_ticks(1);
     timeout_cnt--;
