...
*/
euint32 r,r1=0x0,save_ptr;
//для отладки
File *d_file;
#if 1
euint32 file_read(File *file,euint32 size,euint8 *buf)
{
unsigned long _res_long;  
//------------------------ rec  
      if(_int_rec)
      {
        _res_long = efile_read(size,buf);
        if(_res_long == EMPTY_ID)
          while(1) asm("  NOP  ");
...
//для decrypt
char buf1[512];
euint32 _real_size=0;


if((m_env15==0x01) || (m_env17_ == 0x02))
{
	r=file_fread(file,file->FilePtr,size,buf);
	file->FilePtr+=r;

        
	return(r); //всяка mp3 хрень

}
        save_ptr = file->FilePtr;
//для отладки        
        d_file = file;
        
  if(strstr(file->DirEntry.FileName,"LKF") != NULL)
  {
        while(size != 0)
        {
          if(file->FilePtr < 512)
          {
            r=file_fread(file,0,512,buf1);
          }
          else
          {
            r=file_fread(file,file->FilePtr-(file->FilePtr%512),512,buf1);
          }
          ...
          if(size <= 512)
          {
            memcpy(buf, buf1+(file->FilePtr%512), size);
            if(r<size)
              _real_size += r;
            else
              _real_size += size;
          ...
          {
            memcpy(buf, buf1+(file->FilePtr%512), r-(file->FilePtr%512));
            size -= (r-(file->FilePtr%512));
            buf += (r-(file->FilePtr%512));
            _real_size += (r-(file->FilePtr%512));
            file->FilePtr +=(r-(file->FilePtr%512));
        ...
        file->FilePtr=save_ptr+_real_size;
        r=_real_size;
  } 
  else
  {
...
/*****************************************************************************/
/*****************************************************************************/
euint32 file_read1(File *file,euint32 size,euint8 *buf)
{
...
//----------------------  записи 
    ...
    if(_int_rec)
    {
      if((filename[0] == 2)) //mic rec
      {
        do {
          _id_radio = efile_fopen(_iplay_id, TYPE_MIC, MODE_READ);
          if(_id_radio == EMPTY_ID)
            _iplay_id = _curr_param._id_mic;
        } while(_id_radio == EMPTY_ID);
      }
      if((filename[0] == 1)) //lin rec
      {
        do {
          _id_radio = efile_fopen(_iplay_id, TYPE_LIN, MODE_READ);
          if(_id_radio == EMPTY_ID)
            _iplay_id = _curr_param._id_lin;
        } while(_id_radio == EMPTY_ID);        
      }
      if((filename[0] == 0)) //radio
      {
        _id_radio = efile_fopen(_iplay_id, TYPE_RADIO, MODE_READ);
        do {
          _id_radio = efile_fopen(_iplay_id, TYPE_RADIO, MODE_READ);
          if(_id_radio == EMPTY_ID)
