/***********************************************************************
 * $Id:: lpc32xx_dma_driver.c 8090 2011-09-14 13:57:49Z ing03005       $
 *
  ...
  while (ch < DMA_MAX_CHANNELS)
  {
    ...
    if ((sts_int & _BIT(ch)) != 0)
    {
      ...
      if (dmadrv_dat.cb [ch] != NULL)
      {
        ...
        dmadrv_dat.cb [ch]((dmadrv_dat.pdma->int_err_stat & _BIT(ch)) != 0);

        /* Clear the interrupts */
        dmadrv_dat.pdma->int_tc_clear = _BIT(ch);
        dmadrv_dat.pdma->int_err_clear = _BIT(ch);
      }
      else
      {
...
STATUS dma_init(void)
{
  ...
  if (dmadrv_dat.init == FALSE)
  {
    ...
    CLKPWR->clkpwr_dmaclk_ctrl |= CLKPWR_DMACLKCTRL_CLK_EN;
