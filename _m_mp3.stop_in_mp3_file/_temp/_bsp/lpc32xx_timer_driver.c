/***********************************************************************
 * $Id:: lpc32xx_timer_driver.c 8090 2011-09-14 13:57:49Z ing03005     $
 *
...
void timer_delay_cmn(TIMER_CNTR_REGS_T *pTimer, UNS_32 usec)
{
  ...
  CLKPWR->clkpwr_timers_pwms_clk_ctrl_1 |= CLKPWR_TMRPWMCLK_TIMER_EN(timernum);
                  ...
                  INT_32 arg)
{
  ...
  if (tnum >= 0)
  {
    ...
    if (tmr_init[tnum] == FALSE)
    {
      ...
      /* Enable timer system clock */
      CLKPWR->clkpwr_timers_pwms_clk_ctrl_1 |= CLKPWR_TMRPWMCLK_TIMER_EN(tnum);
