...
#pragma location=0x80000000
static __no_init unsigned char  mp3_stream_buf[15000000];
...
enum mad_flow input(void *data, struct mad_stream *stream)
{
  ...
  if (lb == 0)
  {
    if(0/*((IOCONF_UART_PIN >> 1) & 0x00000001)*/ && !m_env21_) //PORT
    {
      _turnon(0x0);
      while(1)
        asm("nop");
...
if(!m_mylock) //проверим не стои ли lock
{
    ...
    if((m_env22_ & 0x02) == 0x02)
    {
      m_env22_ |= 0x01;
      m_env22_ &= ~0x02;
     //m_place._fpos = mp3_file->FilePtr; PORT
...
//DEBUG    
    ...
    if((_b_vol_down) && !m_env24_ && 1/*!((IOCONF_GPIO_PIN >> 14)&0x00000001)*/) //PORT
    {
      if(0x1B>__k)
      {
        _aic23_svolume(__k);
...
//идем в конец с отступом назад     
    m_env = m_env & ~0x10;
    //mp3_file->FilePtr = mp3_file->FileSize - 50000; PORT
   }
...
//переход по закладкам  
  if((_var55==0x02))
  {
    ...
    if(!_var30)
    {
      //m_place_back._fpos = mp3_file->FilePtr; PORT
#if !_FIX1      
