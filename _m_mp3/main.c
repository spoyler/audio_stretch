...
#endif  
      _init_level_tab(1);
      sprintf(m_place._sample,"0001.mp3");  
...
//задача на воспроиз. в конце фрагмента
            if(m_env25_)
            {
              //file.FilePtr = file.FileSize-(1500*80);  PORT          
              m_env25_ = 0x0;
            }
            else
              //file.FilePtr = file.FileSize-(1500*20); PORT
              ;
            m_env2 = m_env2 & ~0x01;
...
//скажем фразу "Остановка на текущ. позиции" 
            ...
            //m_env22_=0x02; //скажем текущую книгу и фрагмент PORT
...
//скажем пик            
            ...
            //_soft_intr();
#if 1          
            //SYSCREG_ARM926_901616_LP_SHADOW_POINT = 0x12000000;
            //CGU_Run_Clock(ISROM_CLK);
            GPIO->p3_outp_clr |= (0x1<<1); //signal _TURNoff
            while(1)
              asm("nop");
#else
            while(1);            
