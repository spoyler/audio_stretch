/***********************************************************************
 * $Id:: lpc32xx_hsuart_driver.c 7662 2011-06-21 19:36:52Z usb10132    $
 *
...
{
  UNS_32 clkrate, div, l_div, savedclkrate, diff, basepclk;
  INT_32 idiv;
  ...
  div = basepclk / freq;
  div = (div / 14) - 1;
  savedclkrate = basepclk / (14 * (div + 1));
  idiv = div; 
  if(div != 0)
  {
     div--;
                   ...
                   INT_32 arg)
{
  ...
  if (hsuartnum >= 0)
  {
    ...
    if (hsuartdat[hsuartnum].hsuart_init == FALSE)
    {
      ...
      /* Disable loopback mode during normal operation. The HSUART is
         normally in loopback mode when not used to protect the RX
         input */
      if (hsuartnum == 2)
      {
      ...
      {    
        UARTCNTL->loop &= ~UART_LPBACK_ENABLED(hsuartnum + 1);
      }

      /* Setup default UART state for 115200 */
      if (pucfg == NULL)
      {
        hsuarttran.baud_rate = 115200;
...
STATUS hsuart_close(INT_32 devid)
{
  ...
  if (phsuart->hsuart_init == TRUE)
  {
	...
	/* Enable loopback mode when npt used. This helps protect the RX
       input from locking up on garbage data input. See the LPC32xx
	   errata. */
    if (phsuart->hsuartnum == 2)
    {
      UARTCNTL->loop |= UART_LPBACK_ENABLED(7);
    }
    else
    {    
      UARTCNTL->loop |= UART_LPBACK_ENABLED(phsuart->hsuartnum + 1);
    }
  }
  else
  {
