...

{
  ...
  DMAC->dma_chan[dmach].config_ch &= ~DMAC_CHAN_ENABLE; //dis ch
  _lli[0].dma_src = (UNS_32)(mem+sz*sizeof(sz));
  _lli[0].dma_dest = (UNS_32) & I2S0->i2s_tx_fifo;
  _lli[0].next_ctrl =                    DMAC_CHAN_TRANSFER_SIZE(sz) 
  ...
  _lli[0].next_lli = (UNS_32)(&_lli[1]);  
  _lli[1].dma_src = (UNS_32)(mem);
  _lli[1].dma_dest = (UNS_32) & I2S0->i2s_tx_fifo;
  _lli[1].next_ctrl =                    DMAC_CHAN_TRANSFER_SIZE(sz) 
                                      | DMAC_CHAN_SRC_BURST_4
                                      | DMAC_CHAN_DEST_BURST_4 
                                      | DMAC_CHAN_DEST_AHB1
                                      | DMAC_CHAN_SRC_AUTOINC 
                                      ...
                                      | DMAC_CHAN_SRC_WIDTH 
                                      | DMAC_CHAN_DEST_WIDTH;
  _lli[1].next_lli = (UNS_32)(&_lli[0]);   
  cp15_force_cache_coherence((void *) _lli,(void*)((UNS_8 *)_lli + sizeof(_lli) - 1));
  ...
  if (i2sch == I2S_CH0)
  {
    ...
    if (dir == DMAC_CHAN_FLOW_D_M2P)
    {
      ...
      dmaregs->int_tc_clear = _BIT(0);
      dmaregs->int_err_clear = _BIT(0);
      dmaregs->int_tc_clear = _BIT(dmach);
      dmaregs->int_err_clear = _BIT(dmach);      
      DMAC->dma_chan[dmach].src_addr = (UNS_32)(mem);
      ...
      DMAC->dma_chan[dmach].control = DMAC_CHAN_TRANSFER_SIZE(sz) 
                                      ...
                                      | DMAC_CHAN_SRC_WIDTH 
                                      | DMAC_CHAN_DEST_WIDTH;
      DMAC->dma_chan[dmach].lli = (UNS_32)(&_lli[0]);
